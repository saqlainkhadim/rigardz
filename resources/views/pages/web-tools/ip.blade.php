    ...
    <section class="text-gray-700">
        <div class="container px-5 py-24 mx-auto">
            ...
            <form action="{{ route('web.result.ip.lookup')}}" method="post">
                ...
                <button type="submit" class="lg:inline-block py-2 px-6 bg-{{ $config[11]->config_value }}-500 hover:bg-{{ $config[11]->config_value }}-600 text-sm text-white font-bold rounded-l-xl rounded-t-xl transition duration-200">{{ __('Search') }}</button>
            </form>
            ...
            <div class='flex flex-wrap lg:w-full sm:mx-auto sm:mb-2 lg:mt-12'>  
                <div class="rounded-xl border p-5 shadow-md w-full bg-white">
                    ...
                    <div class="mt-4 mb-2">
                        ...
                        <div class="text-sm text-neutral-600"><img width="20" height="20" src="{{ asset('/images/icons/countries/'. mb_strtolower($results['country']['iso_code'] ?? 'not-found')) }}.svg" alt="{{ __($results['country']['names']['en'] ?? 'Not found') }}">{{ __($results['country']['names']['en'] ?? 'Not found') }}</div>
                    </div>

                    {{-- City Name --}}
                    <div class="mt-4 mb-2">
                        <div class="mb-3 text-xl font-bold">{{ __('City Name') }}</div>
                        <div class="text-sm text-neutral-600">{{ __($results['city']['names']['en'] ?? 'Not found') }}</div>
                    </div>

                    {{-- Postal code --}}
                    <div class="mt-4 mb-2">
                        <div class="mb-3 text-xl font-bold">{{ __('Postal code') }}</div>
                        <div class="text-sm text-neutral-600">{{ __($results['postal']['code'] ?? 'Not found') }}</div>
                    </div>

                    {{-- Latitude --}}
                    <div class="mt-4 mb-2">
                        <div class="mb-3 text-xl font-bold">{{ __('Latitude') }}</div>
                        <div class="text-sm text-neutral-600">{{ __($results['location']['latitude'] ?? 'Not found') }}</div>
                    </div>

                    {{-- Longtitude --}}
                    <div class="mt-4 mb-2">
                        <div class="mb-3 text-xl font-bold">{{ __('Longtitude') }}</div>
                        <div class="text-sm text-neutral-600">{{ __($results['location']['longitude'] ?? 'Not found') }}</div>
                    </div>

                    {{-- Timezone --}}
                    <div class="mt-4 mb-2">
                        <div class="mb-3 text-xl font-bold">{{ __('Timezone') }}</div>
                        <div class="text-sm text-neutral-600">{{ __($results['location']['time_zone'] ?? 'Not found') }}</div>
                    </div>
                </div>
            </div>
            @endif
